  ...
  return render(
    <I18nextProvider i18n={i18n}>
      <QueryClientProvider client={client}>
        <MemoryRouter initialEntries={['/app/system/backups']}>
          <SystemBackupsPage />
        </MemoryRouter>
      </QueryClientProvider>
    </I18nextProvider>,
...
describe('SystemBackupsPage', () => {
  it('renders the posture card without crashing once data loads', async () => {
    systemApi.operationalStatus.mockResolvedValue({ backups: { configured: true, note: 'Nightly' } })
    renderPage()
    expect(await screen.findByRole('heading', { name: 'Configured' })).toBeInTheDocument()
  })
})
