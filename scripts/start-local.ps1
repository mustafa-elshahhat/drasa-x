...
.SYNOPSIS
    ...
    PostgreSQL + backend + ai + frontend (unified). Native only (no Docker):
    local PostgreSQL (pg_ctl) + dotnet + uvicorn venv + Vite frontend.
...
Write-Step "Native local stack   (startup gate: $Gate)"
...
$pgBin = Get-PgBin
if (-not $pgBin) { throw "PostgreSQL client tools (pg_ctl) not found. Install PostgreSQL 16." }
